...
vec3 light(vec3 pos, vec3 dir, vec3 l, vec3 lc, vec3 ga, int mtlID) {
    vec3 n = normal(pos);
...
void main() {
    ...
    if (hitSea < hit.x) {
        ...
        vec3 absorptivity = vec3(0.04, 0.033, 0.03) * 0.5;
        ...
        vec3 transmitted = light(pos, rd, -lightDir, lightColor, attenuation, int(hit.y));
        ...
        // Dry surface shading
        radiance = light(pos, ray, -lightDir, lightColor, vec3(1.), int(hit.y));
    }
